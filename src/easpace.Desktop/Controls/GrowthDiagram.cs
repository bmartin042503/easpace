...
/* - Numerikus diagram
 *      - x (idő), y (értékek)
 *      - hozzáadás alsó szekcióban
 *      - pl. Fogyás (céllal: 5 kg), Meditáció (cél nélkül)
 *      - Szerkesztés:
 *          - Cél követése
 *          - (Cél megadása)
 *          - Egység megadása
 *          - Összes adat törlése
 *          - Adatok egyéni törlése és módosítása
 * 
 * - Bináris diagram
 *      - naptári elrendezés/hőtérkép vagy mi
 *      - hozzáadás alsó szekcióban (egyszerűsítve)
 *      - pl. Edzés (teljesítve/nincs teljesítve/üres), Leszokás a dohányzásról (teljesítve/nincs teljesítve/üres)
 *          - Összes adat törlése
 *          - Adatok egyéni törlése és módosítása
 * 
 * - Érték alapú diagram
 *      - körcikk ? + érték hozzáadása (vagy valami cool animáció amivel mutatja a progresst)
 *      - pl. 100 könyv elolvasása
 *          - Összes adat törlése
 *          - Adatok egyéni törlése és módosítása
 *          - Cél megadása
 *
...
public class GrowthDiagram : Control
{
    ...
    private void RenderDiagramLine(DrawingContext context)
    {
        ...
        for (var i = 0; i < ticks; i++)
        {
            ...
            var tickValue = Math.Round(GetValueFromPosY(tickStartPointY));
